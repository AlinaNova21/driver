var bulk = require('./bulk'),
    ...
    os = require('os'),
    util = require('util'),
    runtimeChild, runtimeTimeoutSigkill, runtimeTimeoutSigint, runtimeData, runtimeRestartSignalReceived = false,
    runtimeCache = {},
    roomStatsUpdates = {},
    zlib = require('zlib'),
    worldSize;
            ...
            if (!runtimeChild || !runtimeChild.connected || runtimeRestartSignalReceived) {
                ...
                runtimeChild.on('exit', function(code, signal) {
                    ...
                    if(runtimeChild === this) {
                        runtimeChild = null;
                        console.log(`Runtime worker reset due to child exit: code=${code} signal=${signal} user=${runtimeData.user.username} (${runtimeData.user._id}) promise pending=${promise.isPending()}`);
...
exports.saveUserMemory = function(userId, memory, onlyInRoom) {
    ...
    if(memory.data.length > 2*1024*1024) {
        return Promise.reject('Script execution has been terminated: memory allocation limit reached');
    }
