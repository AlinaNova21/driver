...
process.on('message', (message) => {
    ...
    .then((data) => {
        ...
        try {
            ...
            if(segmentKeys.length > 0) {
                if(segmentKeys.length > 10) {
                    return Promise.reject('Cannot save more than 10 memory segments on the same tick');
                }
                outMessage.memorySegments = {};
                for(var i=0; i<segmentKeys.length; i++) {
                    var key = parseInt(segmentKeys[i]);
                    if(_.isNaN(key) || key < 0 || key > 99) {
                        return Promise.reject(`"${segmentKeys[i]}" is not a valid memory segment ID`);
                    }
                    if(typeof rawMemory.segments[segmentKeys[i]] != 'string'){
                        return Promise.reject(`Memory segment #${segmentKeys[i]} is not a string`);
                    }
                    if(rawMemory.segments[segmentKeys[i]].length > 100*1024) {
                        return Promise.reject(`Memory segment #${segmentKeys[i]} has exceeded 100 KB length limit`);
                    }
        ...
        if(checkOriginalPrototypesResult) {
            return Promise.reject('Security policy violation');
        }
